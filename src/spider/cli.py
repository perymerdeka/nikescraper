...
def get_pages(debug: Optional[bool]=Option(False, help="if this option is true, then debug activated")):
    if debug:
...
def get_product_detail(debug: Optional[bool] = Option(False, help="if this option is true, then debug activated"), url: Optional[str] = Option("", help="Product URL for Nike Website")):
    if debug:
        spider_debug: NikeSpiderDebug = NikeSpiderDebug()
